...
-(void)updateData{
    ...
    NSMutableArray *         array = [[Util getUsingLKDBHelper] searchWithSQL:[NSString stringWithFormat: @"select * from @t  where reporter='%@' and submitStatus!='true' order by rowid desc",[Util getUserName] ]toClass:[DriveCar class]];
    ...
    if ([array count]>0) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            ...
            for ( ii=0; ii<[array count]; ii++) {
                self.driveCar=[array objectAtIndex:ii];
                ...
                if (singleUpFalse==true) {
                    NSLog(@"有上传失败的记录    ii=%d,time=%@",ii,[self getTime]);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [MBProgressHUD hideHUDForView:self.navigationController.view animated:YES];
                        [self showAlert:@"自动上传失败"];
                    });
                    break;
                }
                else if(singleUpFalse==false &&  (isLastNo==true)){
                    NSLog(@"上传结束   ii=%d,time=%@",ii,[self getTime]);
                    dispatch_async(dispatch_get_main_queue(), ^{
                        
                        NSArray *viewControllers = self.navigationController.viewControllers;
                        DriverCarListViewController *rootViewController = [viewControllers objectAtIndex:0];
                        rootViewController.dataArray = [[Util getUsingLKDBHelper] searchWithSQL:[NSString stringWithFormat: @"select * from @t  where reporter='%@' order by rowid desc",[Util getUserName] ]toClass:[_driveCar class]];
...
-(void)updata_Up{
    if (_driveCar.locationUp==nil ) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [MBProgressHUD hideHUDForView:self.navigationController.view animated:YES];
            [self showAlert:@"经纬度没有获取到，请点击提交重试。"];
...
- (void)uploadRequestFailed_Up:(ASIHTTPRequest *)request{
    ...
    NSLog(@"上车结束失败 ii=%d,time=%@",ii,[self getTime]);
    upMessage=@"上车上传失败";
    //HUD.labelText =upMessage;
...
//        }
    ...
    if([downMessage isEqualToString:@"下车上传成功."]){
        ...
        BOOL success =[[Util getUsingLKDBHelper] insertToDB:_driveCar];
        if(!success){
...
- (void)uploadRequestFailed_Down:(ASIHTTPRequest *)request{
    ...
    NSLog(@"下车结束失败 ii=%d,time=%@",ii,[self getTime]);
